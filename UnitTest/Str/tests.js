...
QUnit.test('Str - Str.trim', function (assert) {
    ...
    assert.equal(Str.trimStart(sample, ['k', ' ', 'a']), 'bbcc  ', 'char not in string');
    assert.equal(Str.trimStart(sample, ['k', 'a']), '  aabbcc  ', 'should not remove anything because of leading spaces');
    assert.equal(Str.trimStart(sample, 'x'), sample, 'remove nothing');
    assert.equal(Str.trimStart(sample, ['x', 'k']), sample, 'remove nothing');

    assert.equal(Str.trimEnd(sample), '  aabbcc', 'just space');
    assert.equal(Str.trimEnd(sample, [' ', 'c']), '  aabb', 'remove multiple chars');
    assert.equal(Str.trimEnd(sample, [' ', 'c', 'b']), '  aa', '3 chars');
    assert.equal(Str.trimEnd(sample, ['a', ' ', 'c', 'b']), '', 'nothing left');
    assert.equal(Str.trimEnd(sample, ['k', ' ', 'a']), '  aabbcc', 'char not in string');
    assert.equal(Str.trimEnd(sample, 'x'), sample, 'remove nothing');
    assert.equal(Str.trimEnd(sample, ['x', 'k']), sample, 'remove nothing');

    var regex = '/[\\*??';
    assert.equal(Str.trimEnd(regex, ['?', '*', '\\']), '/[', 'regex chars');
});


QUnit.test('Str - Str.parseJson', function (assert) {
    var arr = [], obj = {}, complexObj = {"s": "hello", "i": 10, "nested": {"i": 11, "arr": [1, 2]}},
        arrJson = JSON.stringify(arr), objJson = JSON.stringify(obj), complexObjJson = JSON.stringify(complexObj);

    function testParser(prefix) {
        assert.deepEqual(Str.parseJson(arrJson), arr, prefix + 'arr');
        assert.deepEqual(Str.parseJson(objJson), obj, prefix + 'obj');
        assert.deepEqual(Str.parseJson(complexObjJson), complexObj, prefix + 'complex');
    }

    if (window.JSON){
        testParser('browser JSON');
    }

    delete window.JSON;
    if (jQuery) {
        testParser('jQuery Json');
    }

    delete jQuery;
    if (!window.JSON && typeof jQuery == 'undefined'){
        assert.throws(Str.parseJson(arrJson), 'if no json function found it should throw an exception');
    }
});

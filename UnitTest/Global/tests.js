...
QUnit.test('Global isArray', function (assert) {
    assert.notOk(_JU.isArray());
    assert.notOk(_JU.isArray(new Date()));
    assert.notOk(_JU.isArray(0));
    assert.notOk(_JU.isArray({}));
    assert.notOk(_JU.isArray({test: 'array'}));

    assert.ok(_JU.isArray([]));
    assert.ok(_JU.isArray([1, 2]));
});


QUnit.test('Global getAttr', function (assert){
    var complexObj = {
        '10': 'ten',
        i: 100,
        s: "hello",
...
QUnit.test('Global setAttr', function (assert)
{

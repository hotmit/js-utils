...
result.push(arr.slice(i,i+chunkSize));return result;};Arr.trim=function(arr,callback){var i,result=[];if(!arr){return result;}
callback=callback||function(elm){return!Str.empty(elm);};for(i=0;i<arr.length;i++){var itm=arr[i];if(callback.call(itm,itm)){result.push(itm);}}
return result;};}(typeof window!=='undefined'?window:this,jQuery,JU.__JU.Arr,JU.__JU.Str));(function(global,$,Fn,Utl,Typ){"use strict";Fn.call=function(func,thisArg,argArray){var args=[].slice.call(arguments).splice(2);return Fn.apply(func,thisArg,args);};Fn.apply=function(func,thisArg,argArray){if(func!=undefined&&$.type(func)==='function'){if(argArray==undefined){argArray=[];}
...
if(context[func]&&$.isFunction(context[func])){return context[func].apply(context,argArray);}};Fn.chain=function(thisArg,arrArray,args){var f;args=Array.prototype.splice.call(arguments,2);for(f in args){if(args.hasOwnProperty(f)){if(f!=undefined&&$.type(f)==='function'){f.apply(thisArg,arrArray);}}}};Fn.combine=function(primaryFunc,trailingFunc,thisArgs){return function(){var args=Array.prototype.splice.call(arguments),primaryResult;if(primaryFunc){primaryResult=primaryFunc.apply(thisArgs||this,args)}
trailingFunc.apply(thisArgs||this,args);return primaryResult;};};Fn.combineWithContext=function(context,funcAttr,thisArg,args){var funcList=Array.prototype.splice.call(arguments,3),primaryFunc=Utl.getAttr(context,funcAttr);funcList.splice(0,0,primaryFunc);while(!(funcList.length&&(primaryFunc=funcList.shift()))){}
if(!primaryFunc){return function(){};}
return function(){var callArgs=Array.prototype.splice.call(arguments),result=primaryFunc.apply(thisArg||this,callArgs),secondaryFunc,i;for(i=0;i<funcList.length;i++){secondaryFunc=funcList[i];if(Typ.isFunc(secondaryFunc)){secondaryFunc.apply(thisArg||this,callArgs);}}
return result;}};}(typeof window!=='undefined'?window:this,jQuery,JU.__JU.Fn,JU.__JU.Utl,JU.__JU.Typ));(function(global,$,Stl){"use strict";Stl.add=function(style){if($.isArray(style))
...
return false;};Slct.selectAll=function(selectElement){$(selectElement).find('option').prop('selected',true);};Slct.selectNone=function(selectElement){$(selectElement).find('option').prop('selected',false);};Slct.isEmpty=function(selectElement){return!$(selectElement).find('option').length;};Slct.isMultiple=function(selector){return $(selector).is('[multiple]');};Slct.autoSaveSelection=function(selector,cookieName,region){var $selectBox=$(selector),$region=$(region||'body'),selectedValue=$.cookie(cookieName);if(!Str.empty(selectedValue)){if(Slct.isMultiple($selectBox)){selectedValue=selectedValue.split(',')}
$selectBox.val(selectedValue).change();}
$region.on('change',selector,function(){var selectedValue=$(this).val();if(selectedValue!=null){if(Slct.isMultiple($selectBox)){selectedValue=selectedValue.join(',');}
$.cookie(cookieName,selectedValue);}
else{$.removeCookie(cookieName)}});}}(typeof window!=='undefined'?window:this,jQuery,JU.__JU.Slct,JU.__JU.Str));(function(global,$,Str){"use strict";Str.toHex=function(s){var r='',i;for(i=0;i<s.length;i++){r+=s.charCodeAt(i).toString(16);}
...
return o.toString();};}(typeof window!=='undefined'?window:this,jQuery,JU.__JU.Str));(function(global,$,Str){"use strict";Str.mtx=[121,111,110,101,32,105,115,32,97,32,103,101,110,105,117,115,32,97,116,32,108,101,97,115,116,32,111,110,99,101,32,97,32,121,101,97,114,46,32,84,104,101,32,114,101,97,108,32,103,101,110,105,117,115,101,115,32,115,105,109,112,108,121,32,104,97,118,101,32,116,104,101,105,114,32,98,114,105,103,104,116,32,105,100,101,97,115,32,99,108,111,115,101,114,32,116,111,103,101,116,104,101,114,46,32,226,128,148,32,71,101,111,114,103,32,67,104,114,105,115,116,111,112,104,32,76,105,99,104,116,101,110,98,101,114,103,11];Str.fuzzit=function(s,breakLine){breakLine=!!breakLine;var r='',k=s.length,jk=[3,5,8,13,21,34,55,89,144],j=0,sl=k,ch=0,i,c,ml=Str.mtx.length;for(i=0;i<s.length;i++,k++){c=s.charCodeAt(i)^Str.mtx[k%ml];r+=Str.padLeft(c.toString(16),String.fromCharCode(103),2);ch++;if(ch%20===0&&ch!==0&&breakLine){r+="\r\n";}
if((i+sl)%jk[j]===0){r+=String.fromCharCode(104+(jk[j]+i+sl)%18);j=(j+1)%jk.length;ch++;if(ch%20===0&&ch!==0&&breakLine){r+="\r\n";}}}
return r;};Str.unfuzzit=function(s){s=s.replace(/[^0-9a-gA-G]/g,'');if(s.length%2!==0){return'error';}
var r='',k=s.length/2,i,c,ml=Str.mtx.length;for(i=0;i<s.length;i+=2,k++){c=((s[i]=='g'||s[i]=='G')?'0':s[i])+s[i+1];r+=String.fromCharCode(parseInt(c,16)^Str.mtx[k%ml]);}
return r;};Str.mtx.unshift(69,118,101,114);Str.mtx.pop();}(typeof window!=='undefined'?window:this,jQuery,JU.__JU.Str));(function(global,$,Pref,Str){"use strict";var _defaultOptions={expires:90,path:'/',secure:false};Pref.set=function(name,value,options){var opt=$.extend({},_defaultOptions,options);if(value==undefined){$.removeCookie(name,opt);}
...
clearTimeout(timer);UI.unblock(elm);};function _prompt(title,$content,bootstrapDialogOpts,callback){var opts=$.extend({closeByBackdrop:false},bootstrapDialogOpts,{title:title,message:$content,onshown:Fn.combineWithContext(bootstrapDialogOpts,'onshown',bootstrapDialogOpts,function(){$content.find('.txt-prompt-result').focus();}),buttons:[{label:BootstrapDialog.DEFAULT_TEXTS['CANCEL'],cssClass:'btn-warn',action:function(dialogRef){dialogRef.close();}},{label:BootstrapDialog.DEFAULT_TEXTS['OK'],cssClass:'btn-primary',action:function(dialogRef){if(callback.call(dialogRef,$content.find('.txt-prompt-result').val(),dialogRef)){dialogRef.close();}}}]});if(!UI.Patterns.dependencyCheck(BootstrapDialog,gettext('UI.prompt'),gettext('This function requires BootstrapDialog'))){return;}
BootstrapDialog.show(opts);}
UI.prompt=function(title,bootstrapDialogOpts,callback){var $textbox=$('<div class="form-group"><input type="text" class="txt-prompt-result form-control"></div>');_prompt(title,$textbox,bootstrapDialogOpts,callback);};UI.promptText=function(title,bootstrapDialogOpts,callback){var $textarea=$('<div class="form-group"><textarea class="txt-prompt-result form-control"></textarea></div>');$textarea.find('textarea').css('min-height','270px');_prompt(title,$textarea,bootstrapDialogOpts,callback);};}(typeof window!=='undefined'?window:this,jQuery,JU.__JU.UI));(function(global,$,Patterns,UI,Str,Bs,Fn,Utl,Obj){"use strict";Patterns.formAutoFocus=function(rootElement){var $rootElement=$(rootElement),reqInput=$rootElement.find('.has-error').find('[type="text"], [type="password"], [type="email"], textarea'),input;if(reqInput.length){reqInput.first().focus().caretToEnd();}
...
else if(command=='redirect'){global.location.href=ajaxCommand.options.redirectUrl;}
...
{defaultBlockUiOptions={message:ajaxCommand.message||null,blockTarget:blockTarget,delay:hasSyncAction?300:2000};blockOptions=Utl.getPrefixedOptions(options,'blockUi',defaultBlockUiOptions);UI.block(blockOptions.blockTarget,blockOptions);executeAsyncActions();setTimeout(function(){executeActions();if(!hasSyncAction){UI.unblock(blockOptions.blockTarget);}},blockOptions.delay);}
else if(displayMethod=='bs-dialog')
{defaultBsDialogOpts={title:options.title||gettext('Message'),message:ajaxCommand.message,animate:false,type:ajaxCommand.status=='error'?'type-danger':'type-primary',buttons:[{label:gettext('OK'),cssClass:'btn-primary',action:function(dialog){dialog.close();}}],onhidden:function(){executeActions();}};executeAsyncActions();bsDialogOpts=Utl.getPrefixedOptions(options,'bsDialog',defaultBsDialogOpts);BootstrapDialog.show(bsDialogOpts);}
...
defaultToastrOpts={title:undefined,type:ajaxCommand.status=='error'?'error':'success',closeButton:true,newestOnTop:true,positionClass:'toast-top-right',onHidden:function(){}};executeAsyncActions();toastrOpts=Utl.getPrefixedOptions(options,'toastr',defaultToastrOpts);toastrType=Obj.pop(toastrOpts,'type','success');toastrTitle=Obj.pop(toastrOpts,'title',undefined);toastr[toastrType](ajaxCommand.message,toastrTitle,toastrOpts);executeActions();}
else if(displayMethod=='alert'){executeAsyncActions();alert(ajaxCommand.message);executeActions();}
else{executeAsyncActions();executeActions();}
return ajaxCommand;};Patterns.bsDialogAjax=function(title,ajaxOpts,dialogOptions,shown,hidden,context,dataParser){if(global.BootstrapDialog==undefined){alert('This function required Bootstrap Dialog plugin.');return;}
...
$.ajax(ajaxOpts).done(function(data){var result=Str.parseJson(data,false);if(result===false){var formHtml=dataParser?dataParser.call(context,data):data;$modalDialog.find('.modal-body').empty().append(formHtml);Patterns.formAutoFocus($modalDialog);Fn.apply(shown,$dialogRef,[data]);unblockWaitingScreen();}
